...
pub struct Entity {
    ...
    pub speed: f32,
    /// How fast the unit is turning on the spot, positive to the left -- see
    /// `world::Motion::from_pace`.
    ///
    /// Zero for everyone but the player, and that is an absence rather than a
    /// claim: nothing on the wire says a creature is turning where it stands,
    /// and the player is the one unit whose keys this client is holding.
    pub turning: f32,
