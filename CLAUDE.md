...
| City services | **All six done and confirmed at the window: trainers, flight paths, trade, mail, guilds and the auction house.** Browsing, paging, bidding and cancelling; **mail can be written, sent and deleted** — a compose form off the inbox's "Send Mail" button, and a two-click delete (affordance then confirmation) on an emptied letter. **Outgoing mail can carry up to twelve item attachments** — right-click a bag item while the compose form is open, the same modal gesture trade offering already established, removable with no confirmation until Send actually posts them (not yet confirmed at the window). No sell window, no search box, no sort control |
...
- **An average cannot see a stall, and it will report a healthy number while
  one is happening.** A single frame took **17.3 seconds** -- a trainer window
  reading `Spell.dbc` once per offered row -- and the same line reported
  `128 fps avg`, because 114 other frames that second were fine. The
  worst-frame picker is the only reason it was found; an fps counter, a median
  and a mean are each incapable of it by construction. **The phase breakdown
  then named the cause with no hypothesis spent at all** -- `panels 17342.7` of
  a `redraw 17361.6` -- which is what splitting the bucket buys.
